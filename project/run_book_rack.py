"""
run_book_rack.py - End-to-end EMP pipeline for the book-insertion task.
"""
...
def _scene_from_demo_goal(scene: BookInsertScene, demo: dict) -> None:
...
def step1_demos(sim: PandaSimEnv = None) -> tuple:
    demo_path = os.path.join(OUT, "demos_book_insert.npz")
    ...
    if not demos:
        print(_section("Step 1: Generating demonstrations"))
        if sim is not None:
            scene.spawn_in_sim(sim)

        demos = generate_demonstrations(
            task_spec=BOOK_INSERT_TASK,
            scene=scene,
            N_demos=N_DEMOS,
            seed=SEED,
            sim=sim,
...
def step2_train(demos: list) -> dict:
    print(_section("Step 2: Training EMP"))
    t0 = time.perf_counter()
    data = preprocess_demo(demos[0], N_max=150)

    print(f"  Training data: N={len(data['X'])}  (BIC selects K)")
...
def step5_plots(model, traj_orig, adapted_policies, demos):
    ...
    plot_rollouts(
        ...
        demo=demos[0],
        save_path=os.path.join(OUT, "book_insert_rollouts.png"),
    )


def step6_visualise(demos, model, adapted_policies, scene: BookInsertScene):
...
def main():
    parser = argparse.ArgumentParser(description="Book-insert EMP pipeline")
