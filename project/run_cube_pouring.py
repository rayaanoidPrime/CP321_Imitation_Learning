"""
run_cube_pouring.py — End-to-end EMP pipeline for the cube-pouring task.
"""
...
def _scene_from_demo_goal(scene: CubePourScene, demo: dict) -> None:
    T_goal = np.asarray(demo.get("T_goal"))
    if T_goal.shape != (4, 4):
        return
    pan_pos = T_goal[:3, 3] - np.array([0.0, 0.0, 0.10])
    scene.set_variant(pan_pos=pan_pos)


def step1_demos(sim: PandaSimEnv = None) -> tuple:
    demo_path = os.path.join(OUT, "demos_cube_pour.npz")
    scene = CubePourScene()
    ...
    if not demos:
        print(_section("Step 1: Generating cube-pour demonstrations"))
        if sim is not None:
            scene.spawn_in_sim(sim)

        demos = generate_demonstrations(
            task_spec=CUBE_POUR_TASK,
            scene=scene,
            N_demos=N_DEMOS,
            seed=SEED,
            sim=sim,
            scene_variant_fn=cube_pan_jitter,
        )
...
def step2_train(demos: list) -> dict:
    print(_section("Step 2: Training EMP for cube pouring"))
...
def step5_plots(model, traj_orig, adapted_policies, demos):
    ...
    plot_rollouts(
        ...
        demo=demos[0],
        save_path=os.path.join(OUT, "cube_pour_rollouts.png"),
    )


def step6_visualise(demos, model, adapted_policies, scene: CubePourScene):
...
def main():
    parser = argparse.ArgumentParser(description="Cube-pour EMP pipeline")
