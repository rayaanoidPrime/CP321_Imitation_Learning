"""
run_book_rack_realtime_record.py - Realtime EMP adaptation with keyboard control + GIF recording.

Launches an interactive PyBullet window where you control the bookrack goal position
with your keyboard. Every frame is captured and saved as a GIF when you exit.
...
def run_and_record(args: argparse.Namespace) -> None:
    ...
    with PandaSimEnv(gui=True) as sim:
        ...
        print("Use sliders (mouse) or keys: arrows, U/J, Q/E.")
        print("S = save GIF and exit, X = exit without saving, R = reset.")
        ...
        while True:
            ...
            if _was_triggered(keys, ord("x")) or _was_triggered(keys, ord("X")):
                print("Exiting without saving GIF.")
                break

            if _was_triggered(keys, ord("s")) or _was_triggered(keys, ord("S")):
                print("Saving GIF and exiting...")
                save_gif = True
                break
            ...
            # Capture every Nth frame to keep GIF size manageable
            frame_counter += 1
            if frame_counter % 3 == 0:
                pos_err_cm = np.linalg.norm(T_actual[:3, 3] - x_goal) * 100.0
                label = f"Realtime Book Insert | goal=[{x_goal[0]:.2f}, {x_goal[1]:.2f}, {x_goal[2]:.2f}] | err={pos_err_cm:.1f}cm"
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Realtime EMP adaptation loop with GIF recording")
